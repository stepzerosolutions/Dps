...
{
    protected $xml_;
    protected $xml_index_;
    protected $xml_value_;
     ...
     * @return void
     */
    public function loadMifMessage($xml)
    {
        $p = xml_parser_create();
    ...
    * @param int    $rootindex Index
    *
    * @return array
    */
    public function getElementIndex($element, $rootindex = 0)
    {
        ...
        if ($pos !== false) {
            ...
            $index = $this->getElementIndex($start_path, $rootindex);
            if ($index == 0) {
                // couldn't find first part give up.
                return 0;
            }
            // recursively find rest
            return $this->getElementIndex($remain_path, $index);
        } else {
